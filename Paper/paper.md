...
![a. SSNT geometry and loading conditions, b. and c. Force-displacement curve and damage contours of the final load increment using a coarse mesh and UAL solver.\label{fig:schematics}](combofig_schematic_f-d_contours.png){width=80%}

![Time for the first 5 iterations of the “func_globalstiffness” function in serial, parallel process-based, and parallel thread-based runs using the UAL solver. \label{fig:threads_vs_processes_coarse_dell}](threads_vs_processes_coarse_dell.png){width=55%}
...
![Time comparisons on Dell G15 and Workstation with coarse, medium, and fine mesh sizes using the UAL solver. Note that the fine mesh cannot run with more than 4 workers on these devices due to memory limitations. \label{fig:laptop_vs_workstation_allmesh}](laptop_vs_workstation_allmesh.png){width=95%}
...
\caption{Time for the first 5 iterations measured for all 3 mesh sizes on the Dell G15 laptop and Dell workstation with the UAL solver. Note that the serial runtime is represented by the x-markers, and the time data is represented on a logarithmic scale for clarity.}
\label{fig:sup1}
\end{figure}
...
\caption{For a super fine mesh with the UAL solver, the effect of additional workers is compared to increased memory allocation.}
\label{fig:sup2}
\end{figure}
