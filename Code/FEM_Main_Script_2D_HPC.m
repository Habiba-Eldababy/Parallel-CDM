...
% Create a parallel pool with 6 threads
...
main_file_path='C:\...';                    % Replace with file path
% Enter location of results storage folder
save_path='C:\...\Saved_Files';             % Replace with file path 

cd (main_file_path);

% Enter your model name
model_name = "SSNT_Fine";
...
if RoutineID == 1
    ...
    while abs(u_bar_conv(ref,1)) <= abs(Applied_Displacement_Load(ref,1)) || residual_norm(end)>tolerance
        ...
        % If there are more than 15 failed increments, exit the program
        if failed_counter > 15
    ...
    onepatch_func_plotmesh2(coords,connect_nds,nelem,nodes_prop_mat(:,1),nodes_prop_mat(:,8),nodes_prop_mat(:,9),nelnodes,'interp',model_name,inc_success_counter,last_iteration,increment,loadfactor,solver_string,tangent_string,SolverID);
...
if RoutineID == 2
    ...
    while loadfactor <= 1 && countflaglf == 0 && incrflag < 100
        ...
        [timing_info,J, dofs, Res_F_F_norm, Res_u_norm, last_iteration, history_var_mat, ~, ~, Reactions_x, Reactions_y] = func_NewtonRaphson_DispControl(model_name,RoutineID,dofs_stored,[fixnodes(1:2,:); fixnodes(3,:)*loadfactor],increment,Delastic,history_var_mat_stored,num_elem_at_node,n_hood,weights,tolerance,IsProj,timing_info);
        ...
        inc_time(increment-1, 1) = toc(inc_time_start);
        cd (save_path)
        save(file_name, "inc_time", "-append");
        cd (main_file_path)
        % -----------------------------------------------------------------
        ...
        if (last_iteration ~= max_accept_iter) && (loadfactor > loadfactor_plot_threshold) % (rem(increment,increment_plot_threshold) == 0)
            IsProj = 1;
            [~,~, ~, ~, ~, ~, ~, gausspoints_prop_mat, nodes_prop_mat, ~, ~] = func_NewtonRaphson_DispControl(model_name, RoutineID, dofs, [fixnodes(1:2,:); fixnodes(3,:)*loadfactor], increment, Delastic, history_var_mat_stored, num_elem_at_node, n_hood, weights, tolerance, IsProj,timing_info);
            max_damage = max(gausspoints_prop_mat(:,1));
            onepatch_func_plotmesh2(coords,connect_nds,nelem,nodes_prop_mat(:,1),nodes_prop_mat(:,8),nodes_prop_mat(:,9),nelnodes,'interp',model_name,inc_success_counter,last_iteration,increment,loadfactor,solver_string,tangent_string,SolverID);
        end
