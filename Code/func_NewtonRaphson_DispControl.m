function [J, dofs, Res_F_F_norm, Res_u_norm, iteration, history_var_mat, gausspoints_prop_mat, nodes_prop_mat, Reactions_x, Reactions_y] = func_NewtonRaphson_DispControl(Damage_type,k_damage_parameter,eq_strain_type, RoutineID, dofs, fixnodes_applied, increment, Delastic, history_var_mat, num_elem_at_node, n_hood, weights, tolerance, IsProj, reaction_calc)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if IsProj == 0
    ...
    [~, J, DRdu, ~, ~, ~, history_var_mat, ~, ~, ~] = func_globalstiffness(Damage_type,k_damage_parameter,eq_strain_type,dofs,Delastic,history_var_mat_previousinc,num_elem_at_node,n_hood,weights,0,[],IsProj,RoutineID);
    ...
    [~, ~, ~, Res_F, ~, ~, ~, ~, ~, ~] = func_globalstiffness(Damage_type,k_damage_parameter,eq_strain_type,dofs,Delastic,history_var_mat_previousinc,num_elem_at_node,n_hood,weights,0,[],IsProj, RoutineID);
    ...
    while true
        ...
        [~, J, DRdu, Res_F, F_int, F_ext, history_var_mat, ~, ~, ~] = func_globalstiffness(Damage_type,k_damage_parameter,eq_strain_type,dofs,Delastic,history_var_mat_previousinc,num_elem_at_node,n_hood,weights,0,[],IsProj,RoutineID);
        ...
        if (Res_u_norm(iteration) < tolerance) || (iteration == max_accept_iter)
            ...
            [K, ~, ~, ~, ~, ~, ~, ~, ~, ~] = func_globalstiffness(Damage_type,k_damage_parameter,eq_strain_type,dofs,Delastic,history_var_mat_previousinc,num_elem_at_node,n_hood,weights,0,[],IsProj,RoutineID);
...
elseif IsProj == 1
    ...
    [~, ~, ~, ~, ~, ~, ~, gausspoints_prop_mat, nodes_prop_mat, ~] = func_globalstiffness(Damage_type,k_damage_parameter,eq_strain_type,dofs,Delastic,history_var_mat_previousinc,num_elem_at_node,n_hood,weights,0,[],IsProj,RoutineID);    
