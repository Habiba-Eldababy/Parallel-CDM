...
% If no parallel pool created by user, create a parpool with 3 threads.
if isempty(gcp('nocreate'))
    parpool('Threads',3); % Adjust number of threads
end
...
if RoutineID == 1
    ...
    while abs(u_bar_conv(ref,1)) <= abs(Applied_Displacement_Load(ref,1)) || residual_norm(end)>tolerance
        ...
        % If there are more than 15 failed increments, save and exit the program
